...
_POSTGRES_DB_NAME=${_POSTGRES_DB_NAME:-"datacatchy"}
_POSTGRES_USER=${_POSTGRES_USER:-"datacatuser"}
_POSTGRES_PASSWORD=${_POSTGRES_PASSWORD:-"datacatP@$$"}
...
if [[ "$(cat ${POSTGRES_CLUSTER_DIR}/PG_VERSION)" == '9.3' ]]; then
    ...
    echo "Creating \`pg_restore\`-compatible, compressed backup of the old \`${_POSTGRES_DB_NAME}\` database."
    ...
    su postgres -c "pg_dump -Z1 -Fc ${_POSTGRES_DB_NAME}" > "/srv/backups/${TIME_STAMP}__${_POSTGRES_DB_NAME}.pg_restore"
...
su postgres -c "${POSTGRES_BIN} --single --config-file=${POSTGRES_CONFIG_FILE}" <<< "CREATE USER $_POSTGRES_USER WITH SUPERUSER;" > /dev/null
su postgres -c "${POSTGRES_BIN} --single --config-file=${POSTGRES_CONFIG_FILE}" <<< "ALTER USER $_POSTGRES_USER WITH PASSWORD '$_POSTGRES_PASSWORD';" > /dev/null
su postgres -c "${POSTGRES_BIN} --single --config-file=${POSTGRES_CONFIG_FILE}" <<< "CREATE DATABASE $_POSTGRES_DB_NAME OWNER $_POSTGRES_USER" > /dev/null
...
su postgres -c "psql ${_POSTGRES_DB_NAME} -c \"create extension if not exists postgis; create extension if not exists postgis_topology\""
pg_ctlcluster 9.4 main stop
